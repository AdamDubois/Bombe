...
class I2C:
    ...
    def getI2C(self):
        """
        Demande une lecture I2C à l'adresse ADDR_ESP_DEMANDE et retourne la chaîne de caractères reçue.
    ...
    def sendI2C(self, message):
        """
        Envoyer une chaîne de caractères via I2C à l'adresse ADDR_ESP_COMMANDE.
        La fonction convertit la chaîne en une liste d'octets et utilise i2c_msg.write pour envoyer les données.

        Parameters:
            message (str): La chaîne de caractères à envoyer.
        """
        message = str(message) # Message à envoyer pour indiquer que la séquence est incorrecte, met les DELs en rouge clignotant

        data = [ord(c) for c in message]
        try:
            i2c_msg_write = i2c_msg.write(ADDR_ESP_COMMANDE, data)
            self.bus.i2c_rdwr(i2c_msg_write)
